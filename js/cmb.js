...
(function ($) {
    ...
    CMBMeta = {
         ...
         */
        TimePicker: function () {
            $('.sailen_timepicker').each(function () {
         ...
         */
        DatePicker: function () {
            $('.sailen_datepicker').each(function () {
                $('#' + jQuery(this).attr('id')).datepicker();
				//$('#_sailen_event_start_date').datepicker();
            ...
            $("#ui-datepicker-div").wrap('<div class="sailen_element" />');
        }, // End DatePicker
        /**
         * Initialize color picker
         */
        ColorPicker: function () {
            $('input:text.sailen_colorpicker').each(function (i) {
        ...
        FileUpload: function () {
            var formfield;
            $('.sailen_upload_file').change(function () {
                formfield = $(this).attr('name');
                $('#' + formfield + '_id').val("");
            });
            $('.sailen_upload_button').live('click', function () {
                var buttonLabel;
                formfield = $(this).prev('input').attr('name');
                buttonLabel = 'Use as ' + $('label[for=' + formfield + ']').text();
                tb_show('', 'media-upload.php?post_id=' + $('#post_ID').val() + '&type=file&sailen_force_send=true&sailen_send_label=' + buttonLabel + '&TB_iframe=true');
                return false;
            });
            $('.sailen_remove_file_button').live('click', function () {
            ...
            window.sailen_original_send_to_editor = window.send_to_editor;
            window.send_to_editor = function (html) {
                ...
                if (formfield) {
                    ...
                    if (itemurl.match(image)) {
                        uploadStatus = '<div class="img_status"><img src="' + itemurl + '" alt="" /><a href="#" class="sailen_remove_file_button" rel="' + formfield + '">Remove Image</a></div>';
                    } else {
                        // No output preview if it's not an image
                        // Standard generic output if it's not an image.
                        html = '<a href="' + itemurl + '" target="_blank" rel="external">View File</a>';
                        uploadStatus = '<div class="no_image"><span class="file_link">' + html + '</span>&nbsp;&nbsp;&nbsp;<a href="#" class="sailen_remove_file_button" rel="' + formfield + '">Remove</a></div>';
                    }
                    $('#' + formfield).val(itemurl);
                    $('#' + formfield + '_id').val(itemid);
                    $('#' + formfield).siblings('.sailen_upload_status').slideDown().html(uploadStatus);
                    tb_remove();
                } else {
                    window.sailen_original_send_to_editor(html);
                }
                formfield = '';
            };
        } // End FileUpload
