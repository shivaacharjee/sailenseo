<?php
/*
Plugin Name: Sailenseo by Shiva Acharjee
Plugin URI: http://greencodes.shivaacharjee.com/sailenseo
Description: Insert schema.org microdata into posts and pages
Version: 1.0.0
Author: Shiva Acharjee
Author URI: http://shivaacharjee.com
Text Domain: micro data schema
...
Actions Hooks:
	sailen_sc_register_settings	: runs when the settings are registered
	sailen_sc_options_validate	: runs when the settings are saved ( &array )
	sailen_sc_metabox			: runs when the metabox is outputted
	sailen_sc_save_metabox		: runs when the metabox is saved

Filters:
	sailen_sc_default_settings	: gets default settings values
	sailen_sc_admin_tooltip		: gets the tooltips for admin pages


*/

if(!defined('SS_BASE'))
	define('SS_BASE', plugin_basename(__FILE__) );

if(!defined('SS_VER'))
	define('SS_VER', '1.0.0');


if ( !class_exists( "Sailenseoschema" ) ) :
	class Sailenseoschema
	{
		/**
		 * Constructs a new sailenSchema
		 */
		public function __construct() {
			...
			add_filter( 'sailen_sc_admin_tooltip', array( $this, 'get_tooltips' ) );
			add_filter( 'sailen_sc_default_settings', array( $this, 'get_default_settings' ) );
		...
		public function do_page() {
			...
			<div class="wrap">
				...
				<h2><?php _e('Sailenseo', 'schema'); ?></h2>
				<div class="schema_options">
					<form action="options.php" method="post">
						<?php settings_fields( 'schema_options' ); ?>
						<?php do_settings_sections('schema_options'); ?>
						<p class="submit">
							<input name="Submit" type="submit" class="button-primary" value="<?php esc_attr_e('Update', 'schema'); ?>" />
						</p>
					</form>
		...
		public function save_metabox( $post_id = 0 )
		{
			...
			do_action( 'sailen_sc_save_metabox' );
		}
		...
		function get_tooltip( $key ) {
			$tooltips = apply_filters( 'sailen_sc_admin_tooltip', array() );
		...
		public function register_settings() {
			...
			do_action( 'sailen_sc_register_settings' );
		}
		...
		function options_validate( $input ) {
			do_action_ref_array( 'sailen_sc_options_validate', array( &$input ) );
		...
		public function default_settings( )
		{
			...
			$default = apply_filters( 'sailen_sc_default_settings', array() );
		...
		public function schema_loader( $posts ) {
			...
			foreach ($posts as $post) {
				$meta_check	= get_post_meta($post->ID, '_sailen_schema_load', true);
			...
			if ( empty($meta_check) && $found == true )
				update_post_meta($post->ID, '_sailen_schema_load', true);

			if ( $found == false )
				delete_post_meta($post->ID, '_sailen_schema_load');
	...
	$Sailenseoschema = new Sailenseoschema();
endif;
